...
## 终端模拟器
终端模拟器现在一般都是只unix桌面app,它模拟古老的终端显示器.它将键盘事件转为终端序列通过内核tty模块发送个shell进程,其中控制字符被转义成信号,后端的shell能接收序列绑定特定功能函数,比如bindkey -e使用emacs的按键方式. 它运行前端进程,前端进程能接收所有虚拟号.所以很多GUI程序有文本模式就是基于这个原理.值得注意的是非法的控制序列没办法模拟发送,所以设置快捷键尽量用转义序列,少用控制序列.
