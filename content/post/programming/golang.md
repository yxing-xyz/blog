...
- **golang汇编中传递参数使用寄存器 + 堆栈传参(SP上面空间), 少于10个参数用寄存器, 实际编译后具体架构就不一定了, 所以go汇编还是封装了一层, C是编译成具体架构汇编**

- **golang使用寄存器传递返回值, 超出9个参数部分使用的是caller的栈空间. 也就是说传递参数和返回值都只会使用caller的栈空间或者只使用寄存器**
## 重点语法
